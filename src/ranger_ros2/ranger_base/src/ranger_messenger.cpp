...
void RangerROSMessenger::TwistCmdCallback(
  ...
  if (parking_mode_ && robot_type_ == RangerSubType::kRangerMiniV2) {
    ...
    // Nav2 常同时输出较小 vx 与 wz；旧逻辑 radius<min_turn_radius 即进原地旋转，
    // 会把线速度清零。仅当几乎无前进速度时才用原地旋转模式。
    constexpr double kSpinLinearThreshold = 0.08;
    const bool negligible_forward =
        std::abs(msg->linear.x) < kSpinLinearThreshold;
    if (negligible_forward && radius < robot_params_.min_turn_radius) {
...
double RangerROSMessenger::CalculateSteeringAngle(geometry_msgs::msg::Twist msg,
  ...
  const double phi_i = atan((l / 2) / radius);
  return k * phi_i;
}
