...
#include "ros2_lidar_2d_converter/pointcloud_to_laserscan_node.hpp"
...
{
  target_frame_ = this->declare_parameter("target_frame", "laser");
  ...
  while (rclcpp::ok(context) && alive_.load()) {
    ...
    if (subscription_count > 0) {
      if (!sub_.getSubscriber()) {
        ...
        sub_.subscribe(this, "/rslidar_points", qos.get_rmw_qos_profile());
      }
